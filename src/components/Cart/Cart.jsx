...
function Cart() {
  ...
  const generateOrder = async (e,formData) => {
    e.preventDefault();
    let orden = {};
    orden.buyer = { name: formData.name , email: formData.email, phone: formData.phone };
    orden.total = total;
    ...
    <div>
      {cartList.length === 0 ? (
        ...
        <Card bg="dark" className="m-5">
          ...
          {bool === true ? <Forms formData={formData} setFormData={setFormData} generateOrder={generateOrder} removeCart={removeCart} /> : ''}
